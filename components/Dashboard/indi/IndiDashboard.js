...
const SideNavbar = ({ option, handleOption, menuItems, isSideNavOpen }) => {
  ...
  return (
    <div className={`transition-all rounded-full sm:rounded-none sm:min-h-screen delay-75 duration-200 sm:delay-0 sm:duration-300 sm:w-20 2xl:w-52 fixed right-3 top-14 z-50 sm:z-0 sm:right-0 sm:top-0 ${isSideNavOpen ? 'translate-x-0 ' : 'translate-x-40'} sm:translate-x-0  sm:static flex sm:flex-col gap-2 bg-white/80 dark:bg-slate-900/80 sm:h-full border-r border-slate-200 dark:border-slate-700`}>
      ...
      <div className='h-screen min-h-80 w-full relative flex text-neutral-800 bg-inherit overflow-x-hidden '>
        ...
        <div className={`absolute top-[62px] right-4 text-slate-900 dark:text-slate-300 sm:static sm:hidden  ${isSideNavOpen ? '-translate-x-[8.5rem]' : 'translate-x-0'} transition-transform duration-300 z-50 `}>
          <div className='flex gap-1 justify-center items-center'>
            <button onClick={()=>router.push('/')} className='w-5 h-5 p-1 flex items-center justify-center rounded-full bg-white dark:text-white dark:bg-orange-500 shadow-sm'>
              <Home className='w-4 h-4' />
            </button>
            <button onClick={() => setIsSideNavOpen(!isSideNavOpen)} className='w-5 h-5 flex items-center justify-center rounded-full bg-white dark:bg-slate-900 shadow-sm'>
              <ChevronRight className={`w-4 h-4 ${isSideNavOpen ? '' : 'rotate-180'} transition-transform duration-500 z-50`} />
            </button>
          </div>
        </div>
